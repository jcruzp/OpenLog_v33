/*
 ...
 v3.30 La fecha de los archivos creados se toma del RTC DS1307
 Permite ajustar la fecha con la opcion "time" en el menu
 
 */

#include <Wire.h> //Utilizado para manejar el RTC
...
SerialPort<0, 600, 0> NewSerial; //Ajustado a 600 para que el buffer no tenga problema con el RTC
...
#define MAX_COUNT_COMMAND_LINE_ARGS 6

// Esta es la direccion I2C del RTC
...
//Lectura de la fecha y hora actual
void LeerRTC(byte* second, byte* minute, byte* hour, byte* day, byte* month, byte* year){
...
void dateTime(uint16_t* date, uint16_t* time) {
  ...
  LeerRTC(&second, &minute, &hour, &day, &month, &year);

  // return date using FAT_DATE macro to format fields
  *date = FAT_DATE(2000+year, month, day);
...
void setup(void)
{
  ...
  //ADCSRA &= ~(1<<ADEN); //Disable ADC Habilitado para que funcione el RTC que esta en A5 y A4
  ACSR = (1<<ACD); //Disable the analog comparator
  //DIDR0 = 0x3F; //Disable digital input buffers on all ADC0-ADC5 pins Habilitado por el RTC
  DIDR1 = (1<<AIN1D)|(1<<AIN0D); //Disable digital input buffer on AIN1/0
  //
  //power_twi_disable(); Por el RTC
  power_timer1_disable();
  power_timer2_disable();
  //power_adc_disable(); Por el RTC
  ...
  //Para utilizar microSD de SparkFun chipselect es pin 8
  //if (!card.init(SPI_FULL_SPEED,8)) systemError(ERROR_CARD_INIT);
...
//ERROR
     ...
     */
#endif //INCLUDE_SIMPLE_EMBEDDED
    else if(strcmp_P(command_arg, PSTR("settime")) == 0)
    {

      byte minute=0, hour=0, day=0, month=0, year=0;

      if (count_cmd_args() == 6)  // has no arguments
      {
        if ((command_arg = get_cmd_arg(1)) != 0) 
          if ((command_arg = is_number(command_arg, strlen(command_arg))) != 0) 
            day = strtolong(command_arg);


        if ((command_arg = get_cmd_arg(2)) != 0) 
          if ((command_arg = is_number(command_arg, strlen(command_arg))) != 0) 
            month = strtolong(command_arg);    


        if ((command_arg = get_cmd_arg(3)) != 0) 
          if ((command_arg = is_number(command_arg, strlen(command_arg))) != 0) 
            year = strtolong(command_arg);    


        if ((command_arg = get_cmd_arg(4)) != 0) 
          if ((command_arg = is_number(command_arg, strlen(command_arg))) != 0) 
            hour = strtolong(command_arg);    


        if ((command_arg = get_cmd_arg(5)) != 0) 
          if ((command_arg = is_number(command_arg, strlen(command_arg))) != 0) 
            minute = strtolong(command_arg);    


        Wire.beginTransmission(DS1307_I2C_ADDRESS); // Empieza transmisión.
        Wire.write(0x00);
        Wire.write(decToBcd(0x01));   // 0 a bit 7  Inicia el reloj
        Wire.write(decToBcd(minute));
        Wire.write(decToBcd(hour));     // Si quiere 12 horas am/pm tiene que poner 
    ...
    else if(strcmp_P(command_arg, PSTR("gettime")) == 0)
    {
      // fetch the time
      byte second, minute, hour, day, month, year;
      ...
      NewSerial.print(day, DEC);
      NewSerial.print(F("-"));
      NewSerial.print(month, DEC);
      NewSerial.print(F("-"));
      NewSerial.print(2000+year, DEC);
      NewSerial.print(F(" "));
      NewSerial.print(hour, DEC);
      NewSerial.print(F(":"));
      NewSerial.print(minute, DEC);
      NewSerial.print(F(":"));
      NewSerial.println(second, DEC);
#ifdef INCLUDE_SIMPLE_EMBEDDED
          command_succedded = 1;
#endif
    } 
...
void print_menu(void)
{
  ...
  //Manejo del RTC
  NewSerial.println(F("settime DD MM AA HH MM: Set time to RTC"));
  NewSerial.println(F("gettime\t\t\t: Get time from RTC"));
}
